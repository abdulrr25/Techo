...
The flow rate is stored on-chain when the gig is created:
```javascript
...
Room creation happens server-side (API key kept secret):
...
> **Important:** Set `HUDDLE_API_KEY` as a **non-public** environment variable (server-side only). The `NEXT_PUBLIC_*` variables are client-exposed.
...
git clone https://github.com/your-username/techo.git
cd techo
...
<sub>Teacho is a testnet project. All transactions use Base Sepolia testnet ETH (no real value).</sub>
